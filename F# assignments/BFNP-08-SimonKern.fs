...
let rec setEval1 e : int Set =
    ...
    | Prim1(op, e1) ->
        let s1 = setEval1 e1
        let xss = Set.map(fun v1 -> opEvalSet1 op v1) s1
        Set.unionMany xss
    | Prim3(op, e1, e2, e3) ->
        let s1 = setEval1 e1
...
let opEvalTrace1 op (v1:int) : int trace = 
    match op with 
    | "ABS" -> (["ABS"], System.Math.Abs v1);;
...
let opEvalOptionTrace op (v1: int option) (v2: int option) : int traceoption =
    let p1 = Option.get v1
    let p2 = Option.get v2
    match op with
    | "+" -> (["+"], Some(p1 + p2))
    | "*" -> (["*"], Some(p1 * p2))
    | "/" -> if Option.get v2 = 0 then (["/"], None) else (["/"], Some(p1 / p2));;

let rec optionTraceEval e : int traceoption =
    match e with
    | CstI i -> ([], Some i)
    | Prim (op, e1, e2) ->
        let (trace1, v1) = optionTraceEval e1
        let (trace2, v2) = optionTraceEval e2
        let (trace3, res) = opEvalOptionTrace op v1 v2
        (trace1 @ trace2 @ trace3, res)
...
let optionTraceM = OptionTraceABuilder();; 
        
let rec optionTraceEval2 e : int traceoption =
    match e with
    | CstI i -> optionTraceM { return i}
    | Prim (op, e1, e2) ->
        optionTraceM { let! v1 = optionTraceEval2 e1
                       let! v2 = optionTraceEval2 e2
                       return! opEvalOptionTrace op v1 v2 }
